/**
 * 本文件定义了一个自定义的日志系统，它在 window.electronLog 的基础上进行了封装。
 * 这样做的主要原因有：
 *
 * 1. 统一日志格式：通过封装，我们可以为所有日志添加额外的信息，如调用栈，使日志更加详细和一致。
 *
 * 2. 错误处理增强：特别是对于 error 日志，我们可以确保 Error 对象的所有重要信息都被正确序列化和记录。
 *
 * 3. 灵活性：这种封装方式允许我们在不修改原始 window.electronLog 的情况下，根据需求添加自定义的日志处理逻辑。
 *
 * 4. 类型安全：通过重新定义 window.log，我们可以确保类型检查，提高代码的可靠性。
 *
 * 5. 调试便利：添加调用栈信息可以帮助开发者更快地定位问题所在。
 */
